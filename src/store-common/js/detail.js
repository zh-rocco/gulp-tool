(function() {
  ...
  var Detail = {
    ...
    getCartGoodsCount: function getCartGoodsCount() {
      var count = GOODS_LIST.reduce(function(prev, curr) {
    ...
    // 检查是否存在该商品
    isValidGoods: function isValidGoods() {
      var _this = this;

      return !!GOODS_LIST.find(function(item) {
        var flag = item.id === _this.id;
        if (flag) {
          _this.goodsInfo = item;
        }
        return flag;
      });
    },
    ...
    // 渲染商品信息
    renderGoodsInfo: function renderGoodsInfo() {
    ...
    // 渲染购物车商品数量
    renderCartGoodsCount: function renderCartGoodsCount() {
