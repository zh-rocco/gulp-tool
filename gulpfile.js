...
gulp.task('sprite', () => {
  return gulp
    .src(`src/${PROJECT_NAME}/**/*.png`, { base: `src` })
    .pipe(
      spriteSmith({
...
gulp.task('sass', () => {
  ...
  return gulp
    .src([`src/${PROJECT_NAME}/sass/*.scss`, `!src/${PROJECT_NAME}/sass/_*.scss`])
...
gulp.task('watch:browser', ['sass', 'browser-sync'], () => {
  ...
  gulp
    .watch([`src/${PROJECT_NAME}/*.html`, `src/${PROJECT_NAME}/css/*.css`, `src/${PROJECT_NAME}/js/*.js`])
...
/* ----- 发布阶段 ----- */
const imageCompressFlag = Config.optionsImagemin.open;
...
gulp.task('image', () => {
  ...
  let pluginsImagemin = [imageminPngquant(), imageminJpegRecompress()];
  return gulp
    .src(`src/${PROJECT_NAME}/img/*.+(png|jpg|jpeg|gif|svg)`)
    .pipe(gulpIf(imageCompressFlag, cache(imagemin(pluginsImagemin))))
    .pipe(gulpIf(imageCompressFlag && imageRevFlag, rev()))
    .pipe(gulp.dest(`dist/${PROJECT_NAME}/img`))
    .pipe(gulpIf(imageCompressFlag && imageRevFlag, rev.manifest()))
    .pipe(gulpIf(imageCompressFlag && imageRevFlag, gulp.dest(`rev/${PROJECT_NAME}/img`)));
});
...
/* 发布任务 */
gulp.task('build', () => {
  runSequence('clean', ['sass', 'image', 'copy'], ['css', 'js'], 'html');
});

gulp.task('build:clean', () => {
  runSequence('clean:all', ['sass', 'image', 'copy'], ['css', 'js'], 'html');
});
